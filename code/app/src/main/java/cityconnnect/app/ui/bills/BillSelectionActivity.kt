...
class BillSelectionActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val pendingBillsList = listOf(
            PendingBills("Electricity", 51.0, "2018-12-12", 1,1),
            PendingBills("Internet", 30.0, "2019-5-6", 2, 2),
            PendingBills("Water", 20.0, "2021-8-3", 3, 3)
        )
        // Assuming you have a RecyclerView with id 'pendingBillRecyclerView' in your layout
        val recyclerView: RecyclerView = findViewById(R.id.pendingBillRecyclerView)
        recyclerView.layoutManager = LinearLayoutManager(this)
        val adapter = BillAdapter(pendingBillsList)
